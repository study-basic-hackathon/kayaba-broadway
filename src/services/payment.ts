...
export async function fetchUserPayment(db: AppDatabase, user_id: string) {
  return (await db.select().from(user_payment_providers).where(eq(user_payment_providers.user_id, user_id)).get()) as
    | UserPaymentProvider
    | undefined;
}

export async function fetchProducts(db: AppDatabase, items: { id: string; quantity: number }[]) {
...
export async function createStripeCustomer(stripe: Stripe, db: AppDatabase, user_id: string, email: string): Promise<Stripe.Customer> {
